...
void OPM::TimerA()
{
	if (regtc & 0x80)
	{
		for (int i=0; i<8; i++)
		{
//			ch[i].KeyControl(0);   // Kenjo, キーオフは要らない？
			ch[i].KeyControl(0xf);
		}
	}
}
...
// Kenjo, もうちょい精度欲しいのでfloat版
void OPM::SetVolume(float db)
{
	db = ( db < 20.0f ) ? db : 20.0f;
	if (db > -192.0f)
		fmvolume = int(16384.0 * pow(10, (double)db / 40.0));
	else
		fmvolume = 0;
}
...
void OPM::SetReg(uint addr, uint data)
{
	...
	case 0x08:					// KEYON
		if (1/*!(regtc & 0x80)*/)  // Kenjo, CSM時も個別のキーオンはできる、はず
...
void OPM::SetParameter(uint addr, uint data)
{
	...
	case 3: // 60-7F TL
		op->SetTL(data & 0x7f, 0/*(regtc & 0x80) != 0*/);  // Kenjo, CSMの時でもTLセットしないと、HEAVY NOVAでおかしくなる
		break;
